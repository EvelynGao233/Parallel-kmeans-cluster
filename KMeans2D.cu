...
__global__ void get_dst(float* dst, float* x, float* y, float* mu_x, float* mu_y, int n, int k) {
    ...
    if (i < n && j < k) {
        float dx = x[i] - mu_x[j];
        float dy = y[i] - mu_y[j];
        dst[i * k + j] = dx * dx + dy * dy; // Squared Euclidean distance
    }
}
...
__global__ void clear(float* sum_x, float* sum_y, int* cluster_size, int k) {
    int j = threadIdx.x;
    if (j < k) {
        sum_x[j] = 0.0f;
        sum_y[j] = 0.0f;
...
__global__ void recenter_step1(float* sum_x, float* sum_y, int* cluster_size, float* x, float* y, int* group, int n) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void recenter_step2(float* mu_x, float* mu_y, float* sum_x, float* sum_y, int* cluster_size, int k) {
    int j = threadIdx.x;
...
// K-Means function
void kmeans(int n, int k, int max_iters, float* x, float* y, float* mu_x, float* mu_y, int* group) {
    // Device memory
    ...
    for (int iter = 0; iter < max_iters; iter++) {
        ...
        get_dst<<<gridSize, blockSize>>>(dst_d, x_d, y_d, mu_x_d, mu_y_d, n, k);
        regroup<<<gridSize, blockSize>>>(group_d, dst_d, n, k);
        clear<<<1, k>>>(sum_x_d, sum_y_d, cluster_size_d, k);
        recenter_step1<<<gridSize, blockSize>>>(sum_x_d, sum_y_d, cluster_size_d, x_d, y_d, group_d, n);
        recenter_step2<<<1, k>>>(mu_x_d, mu_y_d, sum_x_d, sum_y_d, cluster_size_d, k);
    }
...
bool read_points(const std::string& filename, std::vector<Point2D>& points) {
...
int main() {
    ...
    for (const auto& filename : filenames) {
        ...
        std::vector<float> mu_x(k), mu_y(k);
        ...
        kmeans(n, k, max_iters, x.data(), y.data(), mu_x.data(), mu_y.data(), group.data());
